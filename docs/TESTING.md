...
- **server/**: cover request validation for `/api/llm`, MCP handler glue, and lambda/local modes in `main.go`/`lambda.go`. Use httptest with fake collectors and MCP servers.
...
3. **Test flow (Go, in `server/llm_integration_test.go`):**
  ...
  - POST `{"prompt":"hello"}` to `/api/llm` and assert the response JSON has a non-empty `result` and status 200.
